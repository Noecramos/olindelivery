        ...
        <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center bg-black/60 backdrop-blur-sm animate-fade-in p-0 md:p-4">
            ...
            >
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    ...
                    {/* Options */}
                    {product.options && product.options.map((opt: ProductOption, idx: number) => (
                        <div key={idx} className="space-y-3 pb-4 border-b border-gray-100">
                            <div className="flex justify-between items-center">
                                ...
                                {opt.max && <span className="text-xs text-gray-400">Max: {opt.max}</span>}
                            </div>

                            <div className="space-y-2">
                                {opt.values.map((val: OptionValue, vIdx: number) => {
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                <input
                                                    ...
                                                    checked={isSelected}
                                                    onChange={() => handleOptionChange(opt.name, val, opt.type)}
                                                />
