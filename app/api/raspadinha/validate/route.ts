...
export async function POST(req: Request) {
    try {
        ...
        const fallbackCode = process.env.RASPADINHA_MASTER_CODE || '9999';
        ...
        // 2. Validate
        if (inputCode === dbCode || inputCode === fallbackCode) { // Keep Master Code for emergencies

            // 3. If it was the dynamic code (not master), rotate it IMMEDIATELY
            if (inputCode !== fallbackCode) {
            ...
            return NextResponse.json({ success: true, rotated: inputCode !== fallbackCode });
        } else {
